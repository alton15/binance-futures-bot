...
[2단계] ANALYZE (코인별) ─────────────────────────────────────────────
  ...
  │  조건: 확인 지표 ≥4개 AND 강도 ≥ 프로필별 최소 AND MTF ≥1개 → Actionable
  ↓
...
**통과 조건**: 확인 지표 ≥ 4개 (Conservative: ≥5) AND 강도 ≥ 프로필별 최소 AND MTF 확인 ≥ 1개

**시그널 품질 필터** (프로필별 페널티):

| 필터 | Conservative | Neutral | Aggressive | Scalp |
|------|:-----------:|:-------:|:----------:|:-----:|
| MACD 반대 방향 | 거부 (×1.0) | -30% | -15% | -25% |
| 저볼륨 (avg 미만) | 거부 (0.5x) | -15% (0.5x) | 없음 | 거부 (0.8x) |
| BB 방향 충돌 | -20% | -10% | 없음 | -15% |
