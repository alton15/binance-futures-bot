...
[TradingAgents](https://github.com/TauricResearch/TradingAgents)의 멀티 에이전트 트레이딩 펌 패턴에서 영감을 받아 — 적대적 시그널 검증, 상황 메모리, 센티먼트 필터, 멀티 관점 리스크 스코어링, 사후 분석 반성 시스템을 LLM 없이 결정론적 규칙 기반으로 구현했다.
...
- **동적 레버리지**: 변동성 티어 × 신호 강도 × (1 - 드로다운) 기반, 프로필별 1~10배 자동 조절
- **수수료 모델링**: 포지션 사이징에 라운드트립 테이커 수수료(0.08%) 반영
- **거래소 정밀도**: 심볼별 수량/가격 소수점 자릿수를 Binance 마켓 데이터에서 가져와 적용 — 주문 거부 방지 (DOGE=정수, BTC=3자리 등)
- **10-Gate 리스크 관리**: 마진 기반 노출 제한, 일일 손실 한도, 드로다운 차단 등 순차 검증
- **시그널 품질 필터**: MACD 반대·저볼륨·BB 충돌 감지 → 프로필별 강도 감쇄/거부
- **적대적 검증 (Bull/Bear)**: 반대 지표 카운트로 거짓 시그널 필터링 (4개 이상 반대 = 거부)
- **센티먼트 필터**: Fear & Greed Index 역발상 필터 — 공포 시 LONG 부스트, 탐욕 시 SHORT 부스트
- **멀티 관점 리스크**: 3관점 스코어링 (공격적/중립/보수적) → 포지션 사이즈 0.6x~1.1x 조절
- **BM25 상황 메모리**: 과거 매매 상황 저장 → BM25 유사도 검색 → 승률 < 40% 시 포지션 30% 축소
- **Reflection 시스템**: 5가지 차원(RSI/ADX/강도/방향/종료사유)별 사후 분석 → 실행 가능한 인사이트 생성
...
│   │   └── sentiment_filter.py  # Fear & Greed Index 역발상 필터 (30분 캐시)
│   ├── indicators/
│   │   ├── calculator.py        # pandas-ta 기술적 지표 계산 (14개 지표)
│   │   └── signals.py           # 8개 지표 가중 투표 → 매매 신호 생성
│   ├── strategy/
│   │   ├── analyzer.py          # 코인별 종합 분석 + 멀티타임프레임 확인
│   │   ├── adversarial.py       # Bull/Bear 적대적 시그널 검증
│   │   ├── reflection.py        # 사후 매매 패턴 분석 및 학습
│   │   └── orchestrator.py      # Scan → Analyze → Risk → Execute 파이프라인
│   ├── memory/
│   │   └── situation_memory.py  # BM25 기반 매매 상황 메모리
│   ├── risk/
│   │   ├── risk_manager.py      # 10-gate 마진 기반 리스크 검증
│   │   ├── perspectives.py      # 멀티 관점 리스크 스코어링 (3관점)
...
│   ├── db/
│   │   └── models.py            # 10개 테이블 + CRUD (aiosqlite)
│   └── notifications/
│       └── notifier.py          # Discord 웹훅 알림 (2채널)
├── tests/                       # 255개 유닛 테스트 (16개 파일)
├── Dockerfile
...
[2단계] ANALYZE (코인별) ─────────────────────────────────────────────
  ...
  │  센티먼트 필터 (Fear & Greed Index) → 역발상 강도 보정
  │  시그널 품질 필터 (MACD/볼륨/BB) → 강도 보정
  │  적대적 검증 (Bull/Bear) → 반대 지표 4개 이상 시 거부
  │  조건: 확인 지표 ≥ 프로필별 최소 AND 강도 ≥ 프로필별 최소 (MTF: 소프트 패널티) → Actionable
  ↓
[3단계] RISK CHECK (10-Gate) ─────────────────────────────────────────
  │  멀티 관점 스코어링 (공격적/중립/보수적) → 포지션 사이즈 0.6x~1.1x 조절
...
[리스크] RISK CHECK (10-Gate, Scalp 프로필) ──────────────────────────
  │  스캘핑 전용: 마진 기반 제한, 최대 3포지션, 60% 노출, 5~10배 레버리지
  ↓
[매매] EXECUTE ───────────────────────────────────────────────────────
  ↓
...
|------|:------:|----------|-----------|-------------|
| MACD (12/26/9) | 2.0 | 불리시 크로스오버, 히스토그램 확대 양수, 시그널선 상방 | 베어리시 크로스오버, 히스토그램 확대 음수, 시그널선 하방 | 히스토그램 부호와 포지션 불일치 |
...
| 거래량 (SMA 20) | 1.0 | 평균 이상 거래량 + 200 EMA 위 | 평균 이상 거래량 + 200 EMA 아래 | 평균 미만 거래량 |
| ADX (14) | 1.0 | ADX ≥ 15 + 상승 추세 | ADX ≥ 15 + 하락 추세 | ADX < 15 (약한 추세) |
...
**통과 조건**: 확인 지표 ≥ 4개 (Conservative: ≥4, Aggressive: ≥5, Scalp: ≥3) AND 강도 ≥ 프로필별 최소 (MTF는 모든 프로필 소프트 — 하드 게이트 없이 강도 패널티만)

**시그널 품질 필터** (프로필별 페널티):

| 필터 | Conservative | Neutral | Aggressive | Scalp |
|------|:-----------:|:-------:|:----------:|:-----:|
| MACD 반대 방향 | 거부 (×1.0) | -30% | -30% | -20% |
...
**멀티 타임프레임 보정** (모든 프로필 소프트 패널티):
- 15분봉 + 4시간봉 모두 동일 방향 → 강도 × 1.15
- 하나만 동일 방향 → 보정 없음 (기본값)
- 모두 다른 방향 → 강도 × 0.5 (하드 차단 없음 — 감소된 강도가 최소 기준 이상이면 진입 가능)

### 지능형 시그널 처리 (TradingAgents 영감)

[TradingAgents](https://github.com/TauricResearch/TradingAgents)의 멀티 에이전트 패턴에서 영감을 받은 5가지 추가 레이어. 모두 LLM 없이 결정론적 규칙 기반으로 구현:

#### 1. 센티먼트 필터 (Fear & Greed Index)

MTF 확인 후 적용. [alternative.me Fear & Greed Index](https://alternative.me/crypto/fear-and-greed-index/)를 활용한 역발상 필터:

| 센티먼트 | LONG 효과 | SHORT 효과 |
|----------|:---------:|:----------:|
| 극심한 공포 (0-20) | **+10%** 부스트 | -10% 페널티 |
| 공포 (21-35) | **+5%** 부스트 | -5% 페널티 |
| 중립 (36-64) | 변동 없음 | 변동 없음 |
| 탐욕 (65-79) | -5% 페널티 | **+5%** 부스트 |
| 극심한 탐욕 (80-100) | -10% 페널티 | **+10%** 부스트 |

30분 캐시로 API 부하 방지. API 실패 시 거래 차단 없음 (graceful degradation).

#### 2. 적대적 검증 (Bull/Bear Debate)

품질 필터 후 적용. 시그널 방향에 반대하는 지표를 카운트:

- 반대 지표 1개당: **-5%** 강도 페널티
- RSI 극단 반대 (LONG인데 RSI>75, SHORT인데 RSI<25): **-10%** 추가 페널티
- 약한 트렌드 (ADX < 15): 페널티 비례 감소 (반대 의견이 덜 의미있음)
- **4개 이상 반대 지표 → 시그널 거부** (is_actionable = false)

#### 3. 멀티 관점 리스크 스코어링

포지션 사이징 후, 10-gate 리스크 체크 전 적용. 3가지 관점에서 매매 평가:

| 관점 | 가중치 | 핵심 기준 |
|------|:------:|-----------|
| 공격적 (25%) | 시그널 강도, 트렌드, 변동성(기회) |
| 중립 (50%) | 균형 있는 리스크/리워드 |
| 보수적 (25%) | 변동성(위험), RSI 극단, 펀딩비용 |
...
과거 매매 상황을 텍스트로 변환 저장하고 BM25 유사도로 검색:

- 진입 조건 (RSI 구간, ADX 구간, 지표 방향, 키워드) → 텍스트 직렬화
- 포지션 종료 시 결과(P&L, 종료 사유) + 상황 텍스트 DB 저장
- 새 시그널 → 유사 과거 상황 Top-5 검색 → 승률 계산
- **유사 상황 승률 < 40% → 포지션 사이즈 30% 축소**
- 최소 5건 이상 데이터 필요 (콜드 스타트 보호)
- 프로필별 격리 (neutral 메모리가 aggressive에 영향 없음)

#### 5. Reflection 시스템

종료된 포지션을 5가지 차원에서 분석하여 패턴 발견:

| 분석 차원 | 버킷 |
|-----------|-------|
| 진입 시 RSI | oversold / low / neutral / high / overbought |
| 진입 시 ADX | weak / moderate / strong |
| 시그널 강도 | weak / moderate / strong / very_strong |
| 방향 | LONG / SHORT |
| 종료 사유 | take_profit / stop_loss / trailing_stop / ... |

- 버킷별 승률 < 35% → **부정적 인사이트** (진입 전 경고)
- 버킷별 승률 > 65% → **긍정적 인사이트** (확신 강화)
- DB에 인사이트 저장하여 시간이 지날수록 누적 학습
...
|------|:-----------:|:-------:|:----------:|:-----:|
| 건당 리스크 | 1.5% ($1.5) | 2% ($2) | 2% ($2) | 0.8% ($0.8) |
| 동시 포지션 | 3개 | 5개 | 5개 | 3개 |
| 총 마진 노출 | 40% | 60% | 70% | 60% |
| 일일 손실 한도 | 4% ($4) | 6% ($6) | 8% ($8) | 7% ($7) |
| 최대 드로다운 | 10% | 20% | 25% | 20% |
| 레버리지 범위 | 1~3배 | 2~6배 | 3~8배 | 5~10배 |
| 최소 신호 강도 | 0.65 | 0.65 | 0.65 | 0.45 |
| SL 배수 (ATR) | 2.0 | 2.0 | 2.0 | 3.0 |
| TP 배수 (ATR) | 4.0 | 4.0 | 4.0 | 3.5 |
| 트레일링 스톱 | 3.0% | 3.0% | 3.5% | 2.0% |
| 트레일링 활성화 | 1.5x ATR | 1.2x ATR | 1.2x ATR | 1.2x ATR |
| 청산 버퍼 | 30% | 20% | 15% | 15% |
| 최대 보유시간 | 48시간 | 72시간 | 72시간 | 4시간 |
| 분석 타임프레임 | 1h + 15m/4h (소프트 MTF) | 1h + 15m/4h (소프트 MTF) | 1h + 15m/4h (소프트 MTF) | 3m + 1m/5m (소프트 MTF) |
...
| `situation_outcomes` | BM25 상황 메모리 (과거 매매 조건 + 결과) |
| `reflection_insights` | 사후 분석으로 발견된 매매 패턴 |
...
pytest tests/test_leverage_calc.py -v       # 레버리지 + 수수료 + 정밀도 (38개)
...
pytest tests/test_adversarial.py -v         # 적대적 검증 (12개)
pytest tests/test_situation_memory.py -v    # BM25 상황 메모리 (13개)
pytest tests/test_sentiment_filter.py -v    # 센티먼트 필터 (18개)
pytest tests/test_risk_perspectives.py -v   # 멀티 관점 리스크 (20개)
pytest tests/test_reflection.py -v          # Reflection 시스템 (15개)
```
...
| 상황 메모리 | rank-bm25 | BM25 유사도 기반 과거 매매 상황 매칭 |
| HTTP 클라이언트 | httpx | Discord 웹훅 전송, Fear & Greed API |
