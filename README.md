...
- **기술적 분석**: RSI, MACD, 볼린저밴드, EMA(9/21/200), ATR, ADX, 스토캐스틱 — 8개 지표 가중 투표 + NEUTRAL 데드존 필터링
...
- **멀티 타임프레임**: 1시간봉 기본 분석 + 15분봉, 4시간봉 보조 확인 (스캘핑: 3분봉 + 1분봉, 15분봉)
- **7가지 자동 종료**: 손절/익절/트레일링 스톱/신호 역전/청산 근접/펀딩비 과다/최대 보유시간
...
[2단계] ANALYZE (코인별) ─────────────────────────────────────────────
  ...
  │  조건: 확인 지표 ≥4개 AND 강도 ≥0.55 AND MTF ≥1개 → Actionable
  ↓
...
**통과 조건**: 확인 지표 ≥ 4개 AND 강도 ≥ 0.55 AND MTF 확인 ≥ 1개
...
TP 거리    = ATR × tp_atr_multiplier   (R:R = 1:2)
...
| 최소 신호 강도 | 0.70 | 0.60 | 0.55 | 0.55 |
| SL 배수 (ATR) | 2.0 | 1.5 | 1.5 | 1.5 |
| TP 배수 (ATR) | 4.0 | 3.0 | 3.0 | 3.0 |
| 최대 보유시간 | 48시간 | 72시간 | 72시간 | 4시간 |
| 분석 타임프레임 | 1h + 15m/4h | 1h + 15m/4h | 1h + 15m/4h | 3m + 1m/15m |
...
# 개별 모듈
pytest tests/test_db_models.py -v           # DB 모델 (10개)
...
pytest tests/test_position_monitor.py -v    # 트레일링 스톱 + 종료 (6개)
pytest tests/test_scalping.py -v            # 스캘핑 전체 (45+개)
```
